// ============================================================
// ECOMMERCE API - Integrado com Supabase (WG Easy)
// Busca produtos da tabela pricelist_itens do sistema WG Easy
// ============================================================

import { supabase } from '@/lib/customSupabaseClient';

// Configuração alternativa Hostinger (backup/legacy)
const ECOMMERCE_API_URL = "https://api-ecommerce.hostinger.com";
const ECOMMERCE_STORE_ID = "store_01K9MVCE4D35739DPNGFJW7B8J";

// Flag para usar Supabase (WG Easy) ou Hostinger
...
export async function getProducts({ids, offset, limit, order, sort_by, is_hidden, to_date} = {}) {
	// ============================================================
	// SUPABASE: Busca produtos do WG Easy (pricelist_itens)
	// ============================================================
	if (USE_SUPABASE) {
		try {
			...
			// Transformar para o formato esperado pelo ProductsList
			const produtosFormatados = (produtos || []).map((p, index) => {
				const precoEmCentavos = Math.round((p.preco || 0) * 100);

				return {
					id: p.id,
					title: p.nome,
					subtitle: p.descricao || p.fabricante || '',
					ribbon_text: p.fabricante || null,
					...
					// Campos extras do WG Easy
					rating: p.avaliacao,
					manufacturer: p.fabricante,
					model: p.modelo,
					external_link: p.link_produto,
				};
			});
			...
			// Fallback para Hostinger se Supabase falhar
		}
	}

	// ============================================================
	// HOSTINGER: Busca produtos da API externa (fallback)
	// ============================================================
...
export async function getProduct(id, {field} = {}) {
	// ============================================================
	// SUPABASE: Busca produto individual do WG Easy
	// ============================================================
	if (USE_SUPABASE) {
		try {
			...
			console.error("Erro na busca Supabase produto:", err);
...
export async function getProductQuantities({fields, product_ids}) {
	// ============================================================
	// SUPABASE: Retorna estoque infinito (sem controle de estoque)
	// ============================================================
	if (USE_SUPABASE) {
		// WG Easy não controla estoque na loja, retorna quantidade fixa
		return {
