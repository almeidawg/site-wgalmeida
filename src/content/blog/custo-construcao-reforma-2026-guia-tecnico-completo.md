---
title: "Quanto Custa Construir e Reformar em 2026: Guia Técnico Completo com Tabelas de Precos SINAPI"
slug: "custo-construcao-reforma-2026-guia-tecnico-completo"
excerpt: "Tabelas completas de custo de construção e reforma 2026, baseadas em 15 anos de dados consolidados, referencia SINAPI, pesquisa de mercado nacional e internacional. Entenda como chegamos nos calculos, quais metricas usamos e como esses numeros impactam sua obra."
image: "/images/banners/ENGENHARIA.webp"
category: "engenharia"
author: "Grupo WG Almeida"
date: "2026-04-04"
featured: true
tags: ["custo construção", "custo reforma", "SINAPI", "preco m2 construção", "2026", "tabela de precos", "orçamento obra", "CUB", "INCC", "metro quadrado construção", "engenharia civil", "arquitetura"]
---

# Custo de Construcao e Reforma 2026: O Guia Tecnico Mais Completo do Brasil

Seja voce proprietario planejando uma reforma, arquiteto elaborando orcamentos, engenheiro validando propostas ou investidor avaliando viabilidade, esta guia foi criado para voce.

Consolidamos **15 anos de dados do mercado de construcao civil**, combinando referencias do SINAPI, CUB regional, pesquisa de campo nacional e comparativos internacionais para entregar as tabelas de preco por m2 mais precisas disponiveis publicamente em 2026.

---

## Como Chegamos Nestes Numeros: Metodologia

### As 4 Fontes de Dados

Nossa base de custos e construida sobre 4 pilares independentes, que se complementam e se validam mutuamente:

#### 1. SINAPI (Sistema Nacional de Pesquisa de Custos e Indices da Construcao Civil)
- Publicado mensalmente pela CEF (Caixa Economica Federal) e IBGE
- Referencia oficial para obras publicas no Brasil
- Cobertura: todos os estados brasileiros
- Atualizacao: mensal (referencia de marco/2026)

#### 2. CUB (Custo Unitario Basico de Construcao)
- Calculado pelo SINDUSCON de cada estado
- Representa o custo de construcao de edificios residenciais padrao
- Nao inclui: terreno, fundacoes especiais, projetos, taxas, incorporacao

#### 3. Banco de Dados Proprio, 15 Anos de Mercado
- **280+ obras executadas** entre 2011 e 2026
- Medicoes reais de servicos executados por etapa
- Separacao entre custo de material e mao de obra
- Dados de producao (m2/dia, hh/unidade) por servico

#### 4. Pesquisa de Mercado Nacional e Internacional
- Comparativos com mercados de Buenos Aires, Lisboa, Miami e Madrid
- Referencias de revistas especializadas (AU, Projeto Design, Architect's Journal)
- Consulta a especificadores e fabricantes de materiais premium

---

## Indices de Referencia 2026

| Indice | Acumulado 12m | Fonte |
|--------|--------------|-------|
| **INCC** (Indice Nacional Custo Construcao) | +5,81% | FGV |
| **CUB Nacional** | +5,43% | SINDUSCON Nacional |
| **IPCA** (Inflacao Geral) | +4,83% | IBGE |
| **IGPM** | +6,12% | FGV |

> Todos os valores neste guia estao em reais de **marco/2026**, corrigidos pelo INCC.

---

## Tabela 1: Custo de Construcao Nova por Padrao

### Residencial, Sao Paulo Capital (Referencia SP)

| Padrao | Custo M2 Minimo | Custo M2 Referencia | Custo M2 Maximo | CUB Equivalente |
|--------|----------------|---------------------|-----------------|-----------------|
| **Popular** | R$ 1.850 | R$ 2.200 | R$ 2.800 | R-1/PP4 |
| **Medio** | R$ 2.600 | R$ 3.100 | R$ 3.800 | R-1/R4 |
| **Alto Padrao** | R$ 3.800 | R$ 4.800 | R$ 6.200 | PP-4N |
| **Luxo** | R$ 5.500 | R$ 7.200 | R$ 9.500 | Customizado |

**O que esta incluido:**
- Fundacoes convencionais (sapata, radier, bloco)
- Estrutura (concreto armado ou aco, conforme projeto)
- Vedacao (alvenaria ou drywall)
- Cobertura
- Instalacoes eletricas e hidraulicas completas
- Revestimentos de piso e parede conforme padrao
- Esquadrias (janelas e portas) conforme padrao
- Acabamento final conforme padrao

**O que NAO esta incluido:**
- Terreno
- Projetos (arquitetonico, estrutural, hidrossanitario, eletrico)
- Fundacoes especiais (estacas, radier reforado)
- Taxas municipais e aprovacoes
- Paisagismo e areas externas
- Automacao, som e outros opcionais
- Mobiliario planejado

### Composicao do Custo por Categoria (Medio)

| Categoria | % do Custo Total | R$/m2 Referencia |
|-----------|-----------------|------------------|
| Materiais | 56% | R$ 1.736 |
| Mao de obra | 38% | R$ 1.178 |
| BDI (beneficios e despesas indiretas) | 6% | R$ 186 |
| **Total** | **100%** | **R$ 3.100** |

---

## Tabela 2: Custo de Reforma por Nivel

### Reforma Residencial, Apartamento Sao Paulo

| Nivel de Reforma | Custo M2 Minimo | Custo M2 Referencia | Custo M2 Maximo |
|-----------------|----------------|---------------------|-----------------|
| **Basica** | R$ 900 | R$ 1.400 | R$ 2.000 |
| **Intermediaria** | R$ 1.800 | R$ 2.600 | R$ 3.500 |
| **Completa** | R$ 3.000 | R$ 4.200 | R$ 5.800 |
| **Premium Turn Key** | R$ 5.500 | R$ 7.500 | R$ 11.000 |

### O Que Cada Nivel Inclui

| Nivel | Inclui |
|-------|--------|
| **Basica** | Pintura geral, troca de pisos laminados, pequenos reparos, iluminacao simples |
| **Intermediaria** | Basica + revestimentos novos (cozinha e banheiro), eletrica parcial, marcenaria basica |
| **Completa** | Intermediaria + hidraulica completa, layout parcial, marcenaria planejada completa, climatizacao |
| **Turn Key** | Projeto executivo completo + execucao total + marcenaria premium + automacao + acompanhamento integral |

### Custo por Ambiente (Reforma Completa, Padrao Medio)

| Ambiente | Custo Minimo | Custo Referencia | Custo Maximo |
|----------|-------------|------------------|-------------|
| **Sala de estar (30m2)** | R$ 28.000 | R$ 42.000 | R$ 65.000 |
| **Cozinha (15m2)** | R$ 35.000 | R$ 65.000 | R$ 120.000 |
| **Banheiro (6m2)** | R$ 22.000 | R$ 38.000 | R$ 75.000 |
| **Suite (20m2 + BWC 8m2)** | R$ 45.000 | R$ 80.000 | R$ 150.000 |
| **Quarto (15m2)** | R$ 15.000 | R$ 28.000 | R$ 55.000 |
| **Varanda (12m2)** | R$ 18.000 | R$ 35.000 | R$ 65.000 |

---

## Tabela 3: Custo por Servico (Referencia SINAPI mar/2026)

### Revestimentos

| Servico | Unidade | Custo Minimo | Custo Medio | Custo Maximo |
|---------|---------|-------------|------------|-------------|
| Piso ceramica convencional | m2 | R$ 65 | R$ 95 | R$ 140 |
| Piso ceramica porcelanato (ate 60x60) | m2 | R$ 90 | R$ 135 | R$ 200 |
| Piso porcelanato grande formato (120x120+) | m2 | R$ 140 | R$ 220 | R$ 350 |
| Piso madeira (assoalho solido) | m2 | R$ 180 | R$ 280 | R$ 450 |
| Piso madeira (laminado) | m2 | R$ 80 | R$ 130 | R$ 200 |
| Revestimento parede ceramica | m2 | R$ 70 | R$ 110 | R$ 165 |
| Pedra (granito/marmore) | m2 | R$ 220 | R$ 380 | R$ 600 |
| Pintura latex PVA (2 demaos) | m2 | R$ 20 | R$ 32 | R$ 48 |
| Pintura texturizada | m2 | R$ 35 | R$ 58 | R$ 85 |

### Instalacoes Eletricas

| Servico | Unidade | Custo Minimo | Custo Medio | Custo Maximo |
|---------|---------|-------------|------------|-------------|
| Ponto eletrico completo | un | R$ 280 | R$ 420 | R$ 680 |
| Ponto de tomada | un | R$ 180 | R$ 280 | R$ 420 |
| Circuito com disjuntor | un | R$ 450 | R$ 680 | R$ 1.100 |
| Cabeamento estruturado (rede) | ponto | R$ 180 | R$ 280 | R$ 450 |
| Eletrica completa apartamento 80m2 | vb | R$ 18.000 | R$ 32.000 | R$ 55.000 |

### Instalacoes Hidrossanitarias

| Servico | Unidade | Custo Minimo | Custo Medio | Custo Maximo |
|---------|---------|-------------|------------|-------------|
| Ponto de agua fria | un | R$ 350 | R$ 550 | R$ 850 |
| Ponto de esgoto | un | R$ 450 | R$ 680 | R$ 1.100 |
| Troca de louça e metais (banheiro completo) | un | R$ 3.500 | R$ 6.500 | R$ 14.000 |
| Reforma hidraulica completa apto 80m2 | vb | R$ 18.000 | R$ 32.000 | R$ 60.000 |

### Marcenaria Sob Medida

| Servico | Unidade | Custo Minimo | Custo Medio | Custo Maximo |
|---------|---------|-------------|------------|-------------|
| Armario de cozinha (por metro linear) | ml | R$ 1.800 | R$ 3.200 | R$ 6.500 |
| Closet planejado (por metro linear) | ml | R$ 2.200 | R$ 4.000 | R$ 8.500 |
| Bancada porcelanato | ml | R$ 1.200 | R$ 2.200 | R$ 4.500 |
| Marcenaria completa cozinha 12m2 | vb | R$ 35.000 | R$ 65.000 | R$ 140.000 |

---

## Multiplicadores Regionais

Para ajustar os valores de referencia (base SP Capital) para outras regioes:

| Regiao | Multiplicador | Observacao |
|--------|-------------|-----------|
| **Sao Paulo Capital** | 1,00 | Referencia base |
| **Grande SP (outros municipios)** | 0,88 - 0,95 | Depende da distancia do centro |
| **Interior SP (Campinas, Ribeirao)** | 0,82 - 0,90 | Mao de obra e materiais mais baratos |
| **Rio de Janeiro Capital** | 0,92 - 1,08 | Varia muito por bairro |
| **Brasilia** | 0,88 - 0,95 | |
| **Sul (Curitiba, Porto Alegre)** | 0,80 - 0,88 | |
| **Nordeste (Fortaleza, Recife, Salvador)** | 0,72 - 0,82 | |
| **Norte (Manaus, Belem)** | 0,75 - 0,88 | Logistica adicional |

---

## Comparativo Internacional

Para contextualizar os valores brasileiros no cenario global (convertidos para USD/m2 com cambio abr/2026):

| Cidade/Pais | Construcao Padrao Medio (USD/m2) | Equivalente BRL/m2 |
|-------------|----------------------------------|-------------------|
| **Sao Paulo, Brasil** | USD 560 | R$ 3.100 |
| **Buenos Aires, Argentina** | USD 450 | R$ 2.480 |
| **Lisboa, Portugal** | USD 1.800 | R$ 9.900 |
| **Madrid, Espanha** | USD 1.600 | R$ 8.800 |
| **Miami, EUA** | USD 2.200 | R$ 12.100 |
| **Berlim, Alemanha** | USD 2.000 | R$ 11.000 |
| **Dubai, EAU** | USD 1.400 | R$ 7.700 |

> **Conclusao:** O Brasil tem um dos custos de construcao mais competitivos do mundo para projetos de alto padrao, representando uma janela de oportunidade unica para investidores internacionais e para quem quer construir com qualidade global a custos acessiveis.

---

## Fatores que Mais Impactam o Custo Final

### 1. Mao de Obra (38-44% do custo total)

A producao da mao de obra em Sao Paulo:

| Servico | Producao Diaria | Custo Mao de Obra | Custo com Encargos (+72%) |
|---------|----------------|-------------------|--------------------------|
| Assentamento ceramica | 8-12 m2/dia | R$ 280/dia | R$ 480/dia |
| Pintura latex | 25-40 m2/dia | R$ 250/dia | R$ 430/dia |
| Eletricista (ponto) | 3-5 pontos/dia | R$ 320/dia | R$ 550/dia |
| Encanador (ponto) | 2-4 pontos/dia | R$ 350/dia | R$ 600/dia |

### 2. Grau de Dificuldade da Obra

| Condicao | Impacto no Custo |
|----------|-----------------|
| Obra em andares altos (acima do 10o) | +8 a +15% |
| Condominio com restricoes de horario | +5 a +12% |
| Reforma sem desocupacao | +10 a +20% |
| Logistica complexa (materiais pesados) | +5 a +10% |
| Prazo acelerado (acima do ritmo normal) | +15 a +30% |

### 3. Qualidade dos Materiais

A variacoa de preco entre materiais populares e premium para o mesmo item pode ser de 3x a 10x:

| Item | Popular | Medio | Alto Padrao | Luxo |
|------|---------|-------|-------------|------|
| Porcelanato (m2) | R$ 45-80 | R$ 80-200 | R$ 200-500 | R$ 500-2.000 |
| Torneira bancada | R$ 80-200 | R$ 200-600 | R$ 600-1.800 | R$ 1.800-8.000 |
| Janela aluminio (m2) | R$ 280-450 | R$ 450-900 | R$ 900-2.500 | R$ 2.500-8.000 |
| Fechadura | R$ 80-200 | R$ 200-600 | R$ 600-1.500 | R$ 1.500-5.000 |

---

## Como Calcular o Orcamento da Sua Obra

### Formula Simplificada

Para uma estimativa rapida (acuracia de ±20%):

```
Orcamento = Area (m2) × Custo M2 Referencia × Fator Regional × Fator Dificuldade
```

**Exemplo:**
- Reforma completa, 90m2, padrao medio, Sao Paulo Capital, sem restricoes
- Orcamento = 90 × R$ 4.200 × 1,00 × 1,00 = **R$ 378.000**

### Formula Avancada (Para Profissionais)

Para maior precisao, use a composicao por servicos:

1. Quantifique cada servico (m2 de piso, pontos eletricos, m2 de pintura, etc.)
2. Aplique o custo unitario de referencia (tabela SINAPI + pesquisa local)
3. Adicione o BDI da empresa executora (tipicamente 18-28%)
4. Adicione contingencia de projeto (10-15%)
5. Verifique contra o preco por m2 global (sanity check)

---

## Por Que Confiamos Nesses Dados

### 15 Anos de Consolidacao

Desde 2011, a WG Almeida coleta e registra dados de todas as obras executadas:

- **280+ projetos entregues** com medicoes reais
- **R$ 280 milhoes** em obras gerenciadas
- **850+ fornecedores** cadastrados e avaliados
- **12.000+ servicos** com custo unitario rastreado

### Validacao por Multiplas Fontes

Cada valor da nossa tabela e validado contra pelo menos 3 fontes independentes antes de ser publicado. Quando ha divergencia significativa (>15%), investigamos a causa antes de atualizar.

### Atualizacao Mensal

Nossa base e atualizada mensalmente, sempre que o SINAPI publica novos valores ou quando nossa equipe de campo identifica variacoes relevantes no mercado.

---

## Ferramenta de Calculo Online

Quer calcular o custo de construcao ou reforma da sua obra com os dados desta tabela?

Acesse nossa **calculadora AVM e ICCRI integrada**:

- Calculo de custo de construcao nova por padrao (Popular/Medio/Alto/Luxo)
- Custo de reforma estimado por area e nivel
- Comparativo de custo de construir x comprar pronto
- Relatorio PDF com metodologia completa

**Acesse:** [easyrealstate.wgalmeida.com.br/calculo](https://easyrealstate.wgalmeida.com.br/calculo)

---

## Conclusao: Dados Sao a Base de Decisoes Melhores

O mercado de construcao civil brasileiro e frequentemente opaco. Donos de obras sao surpreendidos com custos muito acima do orcado, construtoras sem metodologia perdem controle do BDI e investidores tomam decisoes com base em estimativas simplistas.

Nosso compromisso e trazer transparencia e rigor tecnico para esse mercado. Estes dados sao atualizados regularmente e estao disponiveis publicamente porque acreditamos que um mercado bem informado e um mercado melhor para todos.

**Proximas atualizacoes:** jun/2026 (base SINAPI jun/2026 + CUB 2o trim/2026).

---

*WG Almeida, 15 anos, R$ 280 milhoes em obras, dados reais a servico de decisoes melhores.*

---

**Referencias:**
- SINAPI Marco/2026, CEF/IBGE
- CUB Marco/2026, SINDUSCON-SP
- INCC Acumulado 12m, FGV
- ABNT NBR 14.653 (Avaliacao de Imoveis)
- ABNT NBR 16.280 (Reforma em edificacoes)
- Pesquisa de campo WG Almeida (Jan-Mar/2026)
- Turner & Townsend International Construction Market Survey 2025
- RICS Global Construction Monitor Q4/2025
