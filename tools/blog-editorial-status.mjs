...
import { getBlogManifestEntry, getBlogImageAsset } from '../src/data/blogImageManifest.js';
...
const getManifestEntryType = (entry) => {
  ...
  if (isRemoteAsset(entry)) return 'remote';
  if (isRemoteAsset(entry.hero) || isRemoteAsset(entry.card) || isRemoteAsset(entry.default)) return 'remote';
...
const getCoverage = (post, queueRecord) => {
  ...
  const hasRemoteAsset = Boolean(getBlogImageAsset({ slug: post.slug, category: post.category, variant: 'hero' })) && manifestType === 'remote';
...
console.log(`Published with Unsplash hotlink: ${summary.coverage['published-remote'] || 0}`);
