...
export default function EspecificadorFormPage() {
      ...
      obterPessoa(id)
        .then((pessoa) => {
          if (pessoa) {
            ...
            toast({ title: "Especificador nÍo encontrado", variant: "destructive" });
            navigate("/pessoas/especificadores");
          }
        })
        .catch((error) => {
          console.error("Erro ao carregar especificador:", error);
          toast({ title: "Erro ao carregar dados do especificador", variant: "destructive" });
          navigate("/pessoas/especificadores");
        })
        .finally(() => {
          setLoading(false);
  ...
  async function handleSubmit(data: PessoaInput) {
    try {
      if (isEditMode && id) {
        ...
        toast({ title: "Especificador atualizado com sucesso!" });
      } else {
        await criarPessoa(data);
        toast({ title: "Especificador cadastrado com sucesso!" });
      }
      navigate("/pessoas/especificadores");
    } catch (error: unknown) {
      console.error("Erro ao salvar especificador:", error);
      const message = error instanceof Error ? error.message : "";
      toast({
        title: message || "Erro ao salvar especificador. Verifique os dados e tente novamente.",
        variant: "destructive",
      });
    }
  }
      ...
      <div className={`w-full ${LAYOUT.card} p-4 sm:p-6 md:p-8`}>
        <div className="flex flex-col items-center mb-4 sm:mb-6">
          <h1 className={TYPOGRAPHY.pageTitle}>
            {isEditMode ? "Editar Especificador" : "Novo Especificador"}
          </h1>
        ...
        <div className="flex justify-end mb-3 sm:mb-4">
          <button
            type="button"
            onClick={() => navigate("/pessoas/especificadores")}
        ...
        <div className={LAYOUT.formGrid}>
          <div className={LAYOUT.formFieldFull}>
            <PessoaFormCompleto
              ...
              onCancel={() => navigate("/pessoas/especificadores")}
